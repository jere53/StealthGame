...
#include "MovablePawnSensingComponent.generated.h"
...
/**
 * MovablePawnSensingComponent encapsulates sensory (ie sight and hearing) settings and functionality for an Actor,
 * allowing the actor to see/hear Pawns in the world. It does nothing on network clients.
 * The component can be moved in order to change where the Actor's sensory organs (ie eyes and ears) are.
 */
UCLASS(ClassGroup = (Custom), meta = (BlueprintSpawnableComponent))
class STEALTHGAME_API UMovablePawnSensingComponent : public USceneComponent
{
	...
	/**
	* Arrow that points in the direction that the viewcone is facing. Used for determining where the component is looking at.
	* This arrow is created in the component's constructor and set to be slightly ahead of the viewcone by default
	*/
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = AI)
		class UArrowComponent* FacingDirection;

	//Used to expose the facing director vector to Blueprint so we can draw it
	UPROPERTY(BlueprintReadOnly, Category = Debug)
		FVector FacingDirectionVectorDebug;

	//Vector between the PawnSensingComponent and the Pawn, for debug purposes
	UPROPERTY(BlueprintReadOnly, Category = Debug)
		FVector SelfToOtherDirection;
	
	//Vector with the PawnSensingComponent location, for debug purposes
	UPROPERTY(BlueprintReadOnly, Category = Debug)
		FVector SensorLocationVector;

	//Toggle for enabing debugging
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category = "AI|Components|MovablePawnSensing")
	...
	UFUNCTION(BlueprintCallable, Category = "AI|Components|MovablePawnSensing")
	...
	/** True when we had LoS to a pawn in the previus check, false otherwise */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = AI)
		bool bHadLoSToPawn = false;
	 ...
	 */
	virtual bool CouldSeePawn(const APawn* Other, bool bMaySkipChecks = false);
	...
	virtual	FVector GetSensorRotation() const;
...
public:
	...
	/** Delegate to execute when we lose line of sight to a Pawn. */
	UPROPERTY(BlueprintAssignable)
		FSeePawnDelegate OnUnSeePawn;
...
protected:
	...
	// Broadcasts notification that our sensor lost LoS to a Pawn, using the OnUnSeePawn delegates.
	virtual void BroadcastOnUnSeePawn(APawn& Pawn);
...
inline float UMovablePawnSensingComponent::GetPeripheralVisionAngle() const
{
	return PeripheralVisionAngle;
}

inline float UMovablePawnSensingComponent::GetPeripheralVisionCosine() const
{
